...
export async function generateStaticParams() {
  const { getAuthors } = await import('@/lib/sanity-queries');
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 rounded-lg bg-white p-8 shadow-sm">
          <div className="flex flex-col items-center md:flex-row md:items-start md:gap-8">
            {author.image && (
              <div className="relative mb-6 h-32 w-32 overflow-hidden rounded-full md:mb-0">
                <Image
            ...
            <div className="flex-1 text-center md:text-left">
              <h1 className="mb-2 text-4xl font-bold text-dark-green font-sailers">
                {author.name}
              </h1>
              {author.role && (
                <p className="mb-2 text-xl text-gray-600">{author.role}</p>
              )}
              {author.title && (
                <p className="mb-4 text-lg text-gray-600">{author.title}</p>
              )}
              {author.bio && (
